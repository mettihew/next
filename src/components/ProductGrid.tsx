              ...
              <div className="mt-3 space-y-1">
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i < (product.rating || 0) ? "fill-amber-400 text-amber-400" : "text-gray-300"}`}
                    />
                  ))}
                  {product.reviewCount && (
                ...
                <div className="flex items-baseline gap-2">
                  {product.price && <p className="text-sm font-bold text-gray-900">${product.price}</p>}
                  {product.originalPrice && (
                    <p className="text-xs text-gray-500 line-through">${product.originalPrice}</p>
                  )}
                </div>
                {product.isPrime && <p className="text-xs text-green-600">Prime Delivery</p>}
              </div>
            )}
          </div>
        ))}
