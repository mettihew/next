...
export default function ProductsClient({ productId }: Props) {
  ...
  console.log("ProductsClient received productId:", productId);
  ...
  const renderStars = (rating: number) => {
    ...
    <div className="bg-yellow-100 p-2 rounded text-sm">
        Debug: productId = {productId || "UNDEFINED!"}
      </div>
...
// // Then in your fetch:
//  fetch(`${baseUrl}/api/reviews?productId=${productId}`)
//       .then((res) => res.json())
//       .then((data) => {
//         const revs = data.reviews || [];
//         setReviews(revs);
...
//         <div className="bg-gray-50 rounded-lg shadow-md p-6"> {/* Fixed class name */}
//           <div className="flex items-center mb-6">
//             <div className="text-4xl font-bold mr-4">{average.toFixed(1)}</div>
//             <div>
//               {renderStars(average)}
...
//                   <div className="flex">
//                     <p className="font-semibold">{r.userId?.username || "Anonymous"}</p>
//                     <p className="text-xs text-gray-500 ml-2">{r.rating} ★</p>
//                   </div>
//                   <p className="text-sm text-gray-700">{r.comment}</p>
//                 </div>
//               ))}
